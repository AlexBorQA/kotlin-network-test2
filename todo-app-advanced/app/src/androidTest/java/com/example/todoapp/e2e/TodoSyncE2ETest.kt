...
class TodoSyncE2ETest {
    ...
    fun testFullSyncCycle_CreateTodoOffline_ThenSyncOnline() = runTest {
        // Step 1: Создаем задачу в offline режиме
        ...
        // Проверяем, что задача сохранена локально со статусом PENDING
        ...
        // Step 2: Включаем сеть и запускаем синхронизацию
        ...
        // Настраиваем mock API для успешной синхронизации
        ...
        // Step 3: Проверяем, что задача теперь имеет remoteId
        ...
        // После синхронизации задача должна иметь remoteId
        
        // Step 4: Обновляем задачу и синхронизируем изменения
    ...
    fun testConflictResolution_ServerWinsWithNewerTimestamp() = runTest {
        // Step 1: Создаем задачу локально и на сервере с разными данными
        every { mockNetworkManager.isNetworkAvailable() } returns true
        ...
        val localTodoEntity = com.example.todoapp.data.database.entity.TodoEntity(
            ...
            remoteId = "remote-1",
            updatedAt = System.currentTimeMillis() - 10000 // Старее на 10 секунд
        )
        ...
        val serverTodoDto = TodoDto(
            ...
            priority = "HIGH",
            remoteId = "remote-1",
            updatedAt = System.currentTimeMillis() // Новее
        )
        ...
        // Step 2: Выполняем синхронизацию
        val syncResult = repository.syncWithRemote()
        Assert.assertTrue("Sync should be successful", syncResult.isSuccess)
        
        // Step 3: Проверяем, что серверная версия победила (она новее)
    ...
    fun testBatchSync_MultiplePendingTodos() = runTest {
        // Step 1: Создаем несколько задач в offline режиме
        every { mockNetworkManager.isNetworkAvailable() } returns false
        ...
        // Step 2: Включаем сеть и настраиваем batch sync
        every { mockNetworkManager.isNetworkAvailable() } returns true
        ...
        // Step 3: Выполняем batch синхронизацию
        val syncResult = repository.syncWithRemote()
        Assert.assertTrue("Batch sync should be successful", syncResult.isSuccess)
        
        // Step 4: Проверяем, что все задачи синхронизированы
        val syncedTodos = repository.getAllTodos().first()
        ...
        // Проверяем, что batch запрос был вызван один раз
